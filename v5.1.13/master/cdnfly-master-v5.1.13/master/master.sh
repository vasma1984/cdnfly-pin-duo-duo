...
if [[ $VER == "" ]]; then
  # 获取最新版本
  echo "获取最新版..."
  latest_version=`(curl -s -m 5 'https://update-cn.cdnfly.cn/master/upgrades?latest=1' || curl -s -m 5 'https://update-us.cdnfly.cn/master/upgrades?latest=1') | grep -Po 'v\d+\.\d+.\d+' || true`
