...
conn = Db()
try:
    if not conn.fetchone("SELECT column_name FROM information_schema.columns WHERE table_schema='cdn' and table_name = 'node' AND column_name = 'host';"):
        sql = '''
            alter table node add host varchar(255) default "" after ip;
            alter table node add http_proxy varchar(255) default "{}" after port;  
        '''
        for s in sql.split("\n"):
            if s.strip() == "":
                continue
...
# 安装forcediphttpsadapter
cd /tmp
download "https://dl2.cdnfly.cn/cdnfly/pymodule-master-20211219.tar.gz" "https://us.centos.bz/cdnfly/pymodule-master-20211219.tar.gz" "pymodule-master-20211219.tar.gz"
