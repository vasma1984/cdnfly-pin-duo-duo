...
try:
    # 源站健康检查，可设置间隔时间
    sites = conn.fetchall("select id,health_check from site" )
    for site in sites:
        site_id = site['id']
        health_check = json.loads(site['health_check'])
        health_check['interval'] = 10
        conn.execute("update site set health_check=%s where id=%s ",(json.dumps(health_check), site_id,) )
        conn.commit()

    # 添加默认页规则组指定
    openresty = json.loads(conn.fetchone("select value from config where name='openresty-config' ")['value'])
    openresty['default_page_rule'] = "6"
    conn.execute("update config set value=%s where name='openresty-config'",json.dumps(openresty))
    conn.commit()

    # 开启自动修复
    conn.execute("update config set value='1' where name='record-repair-enable' ")
    conn.commit()
