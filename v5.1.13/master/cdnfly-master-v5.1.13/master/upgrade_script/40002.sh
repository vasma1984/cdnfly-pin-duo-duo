...
try:
    # 403页面
    error_page = conn.fetchone("select value from config where id=46")['value']
    error_page = json.loads(error_page)
    error_page['p403'] = '''<html><head><meta http-equiv="content-type" content="text/html;charset=utf-8"></head><body>您的访问被拒绝</body></html>'''
    conn.execute("update config set value=%s where id=46",json.dumps(error_page))
    conn.commit()
