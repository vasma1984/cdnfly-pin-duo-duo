...
try:
    # 建表
    sql = '''
        create table `site_conf_cache` (`site_id` int(11),`templ_md5` varchar(32),`version` int(11),`data` MEDIUMTEXT,KEY `idx_site_id` (`site_id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;        
    '''
    for s in sql.split("\n"):
        if s.strip() == "":
            continue
