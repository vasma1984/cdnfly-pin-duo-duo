...
layui.define(['laytpl', 'layer'], function(exports){
  ...
  view.req = function(options){
    ...
    var error_show = options.error_show
    if (typeof(error_show) == "undefined") {
      error_show = true
    };
    ...
    options.beforeSend = function (argument) {
      if (loader){
    ...
    return $.ajax($.extend({
      ...
      ,success: function(res){
        ...
        //登录状态失效，清除本地 access-token，并强制跳转到登入页
      ...
      ,error: function(e, code){
        ...
        var error = [
          '请求异常，请重试<br><cite>错误信息：</cite>'+ code +' ' + e.status
          ,debug()
        ].join('');
        view.error(error);
