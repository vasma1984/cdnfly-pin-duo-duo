...
conn = Db()
try:
    # 长连接, proxy_http_version
    sql = '''
        alter table site add ups_keepalive boolean default false after health_check;
        alter table site add ups_keepalive_conn int(3) default 10 after ups_keepalive;
        alter table site add ups_keepalive_timeout int(4) default 60 after ups_keepalive_conn;
        alter table site add proxy_http_version varchar(3) default "1.0" after ups_keepalive_timeout;
