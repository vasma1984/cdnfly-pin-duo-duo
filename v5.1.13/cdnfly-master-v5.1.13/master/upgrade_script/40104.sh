...
conn = Db()
try:
    # 如果设置有spider_allow，设置spider_allow为baidu|google|sm|toutiao|sogou|bing
    sites = conn.fetchall("select id,spider_allow from site")
    for site in sites:
        site_id = site['id']
        spider_allow = site['spider_allow']
        if spider_allow and spider_allow != "":
            conn.execute("update site set spider_allow='baidu|google|sm|toutiao|sogou|bing' where id=%s",site_id )
            conn.commit()
