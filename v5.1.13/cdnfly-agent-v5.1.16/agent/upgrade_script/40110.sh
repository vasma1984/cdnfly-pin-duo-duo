...
upgrade_cmd() {
    ...
    if [[ ! -d /etc/filebeat/ ]]; then
        if check_sys sysRelease ubuntu;then
            cd /tmp
            download "http://dl2.cdnfly.cn/cdnfly/filebeat-7.10.0-amd64.deb" "http://us.centos.bz/cdnfly/filebeat-7.10.0-amd64.deb" "filebeat-7.10.0-amd64.deb"
            dpkg -i filebeat-7.10.0-amd64.deb

        elif check_sys sysRelease centos;then
            cd /tmp
            download "http://dl2.cdnfly.cn/cdnfly/filebeat-7.10.0-x86_64.rpm" "http://us.centos.bz/cdnfly/filebeat-7.10.0-x86_64.rpm" "filebeat-7.10.0-x86_64.rpm"
    ...
    # 修改配置
    master_ip=$(grep "MASTER_IP" /opt/cdnfly/agent/conf/config.py | grep -oE "[0-9.]+")
    es_ip=$(grep "ES_IP" /opt/cdnfly/agent/conf/config.py | grep -oE "[0-9.]+")

    es_pwd=$(curl http://$master_ip:88/agent/es.txt)

    sed -i "s/192.168.0.30/$es_ip/" /opt/$dir_name/agent/conf/filebeat.yml
    sed -i "s/ES_PWD/$es_pwd/" /opt/$dir_name/agent/conf/filebeat.yml

}

# 定义版本
version_name="v4.1.10"
version_num="40110"
