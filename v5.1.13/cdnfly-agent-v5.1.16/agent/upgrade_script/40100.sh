...
upgrade_cmd() {
    # 加软链接
    if [[ ! -f "/usr/local/openresty/libmaxminddb/bin/mmdblookup" ]]; then
        mkdir -p /usr/local/openresty/libmaxminddb/bin/
        cd /usr/local/openresty/libmaxminddb/bin/
        ln -s /usr/local/bin/mmdblookup
    fi
