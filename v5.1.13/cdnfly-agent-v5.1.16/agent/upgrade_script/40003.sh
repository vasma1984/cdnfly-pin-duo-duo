...
# 安装redis
cd /usr/local
redis_tar_name="redis-$(get_sys_ver)-20200714.tar.gz"
download "http://10268950.d.yyupload.com/down/10268950/cdnfly/$redis_tar_name" "http://us.centos.bz/cdnfly/$redis_tar_name" "$redis_tar_name"
tar xf $redis_tar_name

# 下载新openresty
cd /usr/local
if [[ ! -f "/usr/local/openresty/nginx/sbin/nginx.old-20200714" ]]; then
    \cp /usr/local/openresty/nginx/sbin/nginx /usr/local/openresty/nginx/sbin/nginx.old-20200714
fi

## 备份配置文件
\cp /usr/local/openresty/nginx/conf/nginx.conf /tmp/

openresty_tar_name="openresty-$(get_sys_ver)-20200712.tar.gz"
download "http://10268950.d.yyupload.com/down/10268950/cdnfly/$openresty_tar_name" "http://us.centos.bz/cdnfly/$openresty_tar_name" "$openresty_tar_name"
tar xf $openresty_tar_name

## 还原配置文件
\cp /tmp/nginx.conf /usr/local/openresty/nginx/conf/nginx.conf
echo "/usr/local/openresty/hiredis/lib/" > /etc/ld.so.conf.d/hiredis.conf
ldconfig

# 升级nginx.conf
sed -i '/access_log off/i\location ~ /purge/(.*) {\nselective_cache_purge_query "$1";\n}' /usr/local/openresty/nginx/conf/nginx.conf
sed -i '/\/purge\//i\merge_slashes off;' /usr/local/openresty/nginx/conf/nginx.conf
sed -i '/514.html/a\selective_cache_purge_redis_unix_socket "/var/run/redis.sock";\nselective_cache_purge_redis_database 1;' /usr/local/openresty/nginx/conf/nginx.conf
