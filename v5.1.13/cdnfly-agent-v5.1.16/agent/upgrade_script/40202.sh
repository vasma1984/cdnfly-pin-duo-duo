...
upgrade_cmd() {
# 取消缓存插件
sed -i '/selective_cache_purge_redis/d' /usr/local/openresty/nginx/conf/nginx.conf
sed -i '/\/purge\//,+2d' /usr/local/openresty/nginx/conf/nginx.conf

## 替换openresty
# 下载新openresty
cd /usr/local
if [[ ! -f "/usr/local/openresty/nginx/sbin/nginx.old-20210218" ]]; then
    \cp /usr/local/openresty/nginx/sbin/nginx /usr/local/openresty/nginx/sbin/nginx.old-20210218
fi

## 备份配置文件
\cp /usr/local/openresty/nginx/conf/nginx.conf /tmp/

openresty_tar_name="openresty-$(get_sys_ver)-20210218.tar.gz"
download "http://us.centos.bz/cdnfly/$openresty_tar_name" "http://us.centos.bz/cdnfly/$openresty_tar_name" "$openresty_tar_name"
