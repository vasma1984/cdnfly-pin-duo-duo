...
if os.path.exists("/usr/local/openresty/nginx/conf/vhost/health_check.json"):
    with open("/usr/local/openresty/nginx/conf/vhost/health_check.json") as fp:
        data = fp.read()

    health_check = json.loads(data)
    for i in range(len(health_check)):
        health_check[i]["interval"] = 10

    with open("/usr/local/openresty/nginx/conf/vhost/health_check.json","w") as fp:
        fp.write(json.dumps(health_check))
